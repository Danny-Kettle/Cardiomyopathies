...
import { sendEmailVerification, emailSender } from '../firebase/emailVerification';
...
import {emailSender} from '../firebase/emailVerification';
  ...
  handleSubmit() {
    ...
    if (this.isSignup) {
      ...
      createUserWithEmailAndPassword(firebaseAuthentication, this.email, this.password)
        ...
        .then(()=> {
          return emailSender(this.email)
          //sends email to newly registered user to verify email address
        })
        .then(() => {
          // show success alert
